...
@router.get('/analytics/battles/{battle_id}/ml-risk', response_model=BattleMlRiskResponse)
async def get_battle_ml_risk_endpoint(
    battle_id: str,
    viewer: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_async_session),
) -> BattleMlRiskResponse:
    ...
    payload = await get_battle_ml_risk(session, battle_id=battle_id)
    if payload is None:
        raise HTTPException(status_code=404, detail='Баттл не найден')
